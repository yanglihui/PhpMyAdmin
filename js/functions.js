...
function require_message_code(){
	var parma = {};
	parma.pma_user 	= $("#input_user").val();
	parma.pma_phone = $("#input_phone").val();
	if(parma.pma_user.length >0 && parma.pma_phone.length >0){
		console.log(parma);
		$.post("index.php",parma,function(result){
			console.log(result);
			if(result.error == 0){
				$("#ajax_message").text(result.message);
			}else{
				$("#ajax_message").text(result.message);
			}
		  },"json");
