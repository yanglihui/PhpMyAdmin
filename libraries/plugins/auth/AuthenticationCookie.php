...
class AuthenticationCookie extends AuthenticationPlugin
{
    ...
    <form method="post" action="index.php" name="login_form"' , $autocomplete ,
        ...
        if(isset($GLOBALS['cfg']['message_verify']['enable']) && $GLOBALS['cfg']['message_verify']['enable']){
            if(isset($_SESSION['pma_verify_passed']) && $_SESSION['pma_verify_passed']){
                echo '
                    <div class="item" style="margin: 10px 0;">
                        <p id="ajax_message" style="color:#EF0808;">该环境已经通过短信验证</p>
                    </div>  
                    ';
            }else{
                echo '
                    <hr style="background-color:#CCCCCC; margin:10px 0;">
                    <div class="item">
                        <label for="input_user">用户：</label>
                        <input type="text" name="pma_user" id="input_user" value="" size="24" class="textfield">
                    </div>
                    <div class="item">
                        <label for="input_phone">手机号：</label>
                        <input type="text" name="pma_phone" id="input_phone" value="" size="24" class="textfield">
                    </div>
                    <div class="item">
                        <label for="input_phone_code">验证码：</label>
                        <input type="text" name="pma_phone_code" id="input_phone_code" value="" size="24" class="textfield" style="width: 80px;">
                        <input type="button" value="发送验证码" id="input_message_verify" onclick = "require_message_code()">
                    </div>
                    <div class="item" style="margin: 10px 0;">
                        <p id="ajax_message" style="color:#EF0808;"></p>
                    </div>
                    <div class="item" style="margin: 10px 0;">
    ...
    public function authCheck()
    {
        ...
        if (! empty($_REQUEST['pma_username'])) {
            if(isset($GLOBALS['cfg']['message_verify']['enable']) && $GLOBALS['cfg']['message_verify']['enable']){
                if(isset($_SESSION['pma_verify_passed']) && $_SESSION['pma_verify_passed']){
                    ...
                    if(!isset($_REQUEST['pma_phone_code']) || !isset($_SESSION['pma_verify_code'])){
                        $conn_error = "请获取并输入验证码";
                        return false;
                    }elseif( (string)$_REQUEST['pma_phone_code'] !== (string)$_SESSION['pma_verify_code'] ){
                        $conn_error = "验证码错误，请重新获取";
                        unset($_SESSION['pma_verify_code']);
                        $_SESSION['pma_verify_passed'] = false;
                        return false;
                    }else{
                        $_SESSION['pma_verify_passed'] = true;
                    }
                }
            }
        ...
        if(isset($GLOBALS['cfg']['message_verify']['enable']) && $GLOBALS['cfg']['message_verify']['enable']){
            $messageConfig = $GLOBALS['cfg']['message_verify'];
            $response = array();
            if (!empty($_REQUEST['pma_user']) && !empty($_REQUEST['pma_phone'])){
                if(isset($messageConfig[$_REQUEST['pma_user']]) && (string)$messageConfig[$_REQUEST['pma_user']] === $_REQUEST['pma_phone']){
                    $min = pow(10, ($messageConfig['bit'] - 1));
                    $max = pow(10, $messageConfig['bit'])-1;
                    $code = rand($min, $max);
                    $number = rand(1, 99);
                    
                    require_once './libraries/plugins/alidayu/TopSdk.php';
                    require_once './libraries/plugins/alidayu/Autoloader.php';
                    $c = new \TopClient();
                    $c->appkey      = $messageConfig['appkey'];
                    $c->secretKey   = $messageConfig['secretKey'];
                    
                    $content = array(
                        "name"      =>  $_REQUEST['pma_user'],
                        "code"      =>  (string)$code,
                        "number"    =>  (string)$number
                    );
                    $req = new \AlibabaAliqinFcSmsNumSendRequest();
                    $req->setExtend($_REQUEST['pma_user']);
                    $req->setSmsType("normal");
                    $req->setSmsFreeSignName($messageConfig['sign']);
                    $req->setSmsParam(json_encode($content));
                    $req->setRecNum($_REQUEST['pma_phone']);
                    $req->setSmsTemplateCode($messageConfig['template']);
                    $resp = $c->execute($req);
                    $_SESSION['pma_verify_passed'] = false;
                   
                    $_SESSION['pma_verify_code'] = $code;
                    $response['error'] = 0;
                    $response['message'] = "验证码已发送，序号：".$number;
                    echo json_encode($response);
                    exit();
                }else{
                    $response['error'] = 1;
                    $response['message'] = "用户名或手机号码错误";
                    echo json_encode($response);
                    exit();
                }
            }
